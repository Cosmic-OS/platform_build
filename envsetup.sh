...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^cosmic_") ; then
        COSMIC_BUILD=$(echo -n $1 | sed -e 's/^cosmic_//g')
        export BUILD_NUMBER=$( (date +%s%N ; echo $COSMIC_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10 )
    else
        COSMIC_BUILD=
    fi
    export COSMIC_BUILD
...
function lunch()
{
    ...
    then
        ...
        T=$(gettop)
        pushd $T > /dev/null
        $T/vendor/cos/build/tools/roomservice.py $product
        popd > /dev/null
        check_product $product
    else
        T=$(gettop)
        pushd $T > /dev/null
        $T/vendor/cos/build/tools/roomservice.py $product true
        popd > /dev/null
    fi
    if [ $? -ne 0 ]
    then
