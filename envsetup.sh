...
function lunch()
{
    ...
    then
        # if we can't find a product, try to grab it off the Cosmic-OS GitHub org
        T=$(gettop)
        cd $T > /dev/null
        vendor/cos/build/tools/roomservice.py $product
        cd - > /dev/null
        check_product $product
    else
        T=$(gettop)
        cd $T > /dev/null
        vendor/cos/build/tools/roomservice.py $product true
        cd - > /dev/null
    fi
    ...
    echo "";
    cat $(gettop)/build/make/cosmic_ascii_logo;
    echo"";
}
